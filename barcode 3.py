...
def generate_barcodes(values, output_dir: str = "barcodes"):
    """Generate a Code‑128 PNG for each *value* (number **or** word), without text."""
    out_dir = Path(output_dir)
    out_dir.mkdir(exist_ok=True)

    generated_files = []
    # Prepare writer, text writing disabled via options on save
    for idx, value in enumerate(values, start=1):
        ...
        try:
            ...
            # Save with write_text disabled
            saved_file = code.save(str(target), options={"write_text": False})
